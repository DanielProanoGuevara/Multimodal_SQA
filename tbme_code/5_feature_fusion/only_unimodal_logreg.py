# -*- coding: utf-8 -*-
"""
Feature fusion (ECG SQIs + PCG SQIs) + 3-class Logistic Regression (OOF-CV)

File
----
Multimodal_SQA/tbme_code/5_feature_fusion/only_unimodal_logreg.py

Purpose
-------
Train and evaluate a 3-class Logistic Regression classifier using *feature fusion* of
...
OUT_DIR = "exp_step5_feature_fusion_only_unimodal_logreg_3class"
...
def merge_ecg_pcg_features(pcg_df: pd.DataFrame, ecg_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Prefix to guarantee no collisions and keep interpretability
...
def merge_with_manual_msqa(fused_df: pd.DataFrame, manual_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # put labels early for readability
...
def save_confusion_matrix_artifacts(y_true, y_pred, labels, display_labels, out_prefix: str) -> None:
    ...
    ax.set_title("Step 5 — Feature fusion (ECG+PCG SQIs) — LogReg OOF Confusion Matrix (%)")
    plt.tight_layout()
...
# manual_label is the 3-class string (for modeling consistency)
...
# Feature columns = all remaining (exclude identifiers + label columns)
non_feature_cols = {"ID", "Auscultation_Point", "mSQA_min", "gt_3class_str", "manual_label", "y"}
feature_cols = [c for c in df.columns if c not in non_feature_cols]

# Drop any rows with missing features (strict completeness)
...
save_confusion_matrix_artifacts(
    ...
    out_prefix="step5_feature_fusion_logreg_cv",
)
...
macro_csv = os.path.join(OUT_DIR, "step5_feature_fusion_logreg_cv_macro_metrics.csv")
...
per_class_csv = os.path.join(OUT_DIR, "step5_feature_fusion_logreg_cv_per_class_metrics.csv")
...
pred_csv = os.path.join(OUT_DIR, "step5_feature_fusion_logreg_oof_predictions.csv")
pred_df.to_csv(pred_csv, index=False)

# %% Minimal console summary
print(f"[Step 5] Saved outputs to: {OUT_DIR}")
print(" - Confusion matrix (% PNG):    step5_feature_fusion_logreg_cv_confusion_matrix_percent.png")
print(" - CV macro metrics:            step5_feature_fusion_logreg_cv_macro_metrics.csv")
print(" - CV per-class metrics:        step5_feature_fusion_logreg_cv_per_class_metrics.csv")
print(" - OOF predictions (per-row):   step5_feature_fusion_logreg_oof_predictions.csv")
