# -*- coding: utf-8 -*-
"""
All features fusion (PCG SQIs + ECG SQIs + alignment_metric*) + 3-class RBF SVM (OOF-CV)

File
----
Multimodal_SQA/tbme_code/5_feature_fusion/all_features_svm.py

Purpose
-------
Train and evaluate a 3-class SVM (RBF kernel) classifier using:
...
OUT_DIR = "exp_step5_all_features_svm_3class"
...
def save_confusion_matrix_artifacts(y_true, y_pred, labels, display_labels, out_prefix: str) -> None:
    ...
    ax.set_title("All features — RBF SVM — OOF Confusion Matrix (%)")
    plt.tight_layout()
...
def merge_pcg_ecg_features(pcg_df: pd.DataFrame, ecg_df: pd.DataFrame) -> pd.DataFrame:
    ...
    merged = pd.merge(
        pcg,
        ecg.drop(columns=["Auscultation_Point"]),
        on=["ID", "_k_point"],
        how="inner",
    )
...
def merge_with_manual_msqa(allfeat_df: pd.DataFrame, manual_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["Trial", "Spot", "mSQA_min"]:
        if c not in m.columns:
            raise ValueError(f"Manual Excel missing required column '{c}'. Found: {list(m.columns)}")
...
# %% Load inputs
print("[All features SVM] Loading PCG SQIs...")
pcg_feat = pd.read_csv(PCG_FEAT_PATH)

print("[All features SVM] Loading ECG SQIs...")
ecg_feat = pd.read_csv(ECG_FEAT_PATH)

print("[All features SVM] Loading alignment metrics PKL...")
align_df = pd.read_pickle(ALIGN_PKL_PATH)

print("[All features SVM] Loading manual labels Excel...")
m_quality = pd.read_excel(MQ_PATH)

# %% Build fused feature table
fused_unimodal = merge_pcg_ecg_features(pcg_feat, ecg_feat)
print(f"[All features SVM] Joined ECG+PCG rows: {len(fused_unimodal)}")

fused_all = merge_alignment_metrics(fused_unimodal, align_df)
print(f"[All features SVM] Added alignment metrics rows: {len(fused_all)}")

merged = merge_with_manual_msqa(fused_all, m_quality)
print(f"[All features SVM] Joined with manual mSQA_min rows: {len(merged)}")
...
save_confusion_matrix_artifacts(
    ...
    out_prefix="all_features_svm_cv",
)
...
macro_csv = os.path.join(OUT_DIR, "all_features_svm_cv_macro_metrics.csv")
...
per_class_csv = os.path.join(OUT_DIR, "all_features_svm_cv_per_class_metrics.csv")
...
pred_csv = os.path.join(OUT_DIR, "all_features_svm_oof_predictions.csv")
pred_df.to_csv(pred_csv, index=False)

# %% Minimal console summary
print(f"[All features SVM] Saved outputs to: {OUT_DIR}")
print(" - Confusion matrix (% PNG):    all_features_svm_cv_confusion_matrix_percent.png")
print(" - CV macro metrics:            all_features_svm_cv_macro_metrics.csv")
print(" - CV per-class metrics:        all_features_svm_cv_per_class_metrics.csv")
print(" - OOF predictions (per-row):   all_features_svm_oof_predictions.csv")
