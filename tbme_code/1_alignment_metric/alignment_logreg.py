# -*- coding: utf-8 -*-
"""
Alignment metric (multimodal) + 3-class Logistic Regression

Purpose
-------
Train and evaluate a 3-class Logistic Regression classifier using only multimodal
alignment-derived features (all columns starting with 'alignment_metric') and
manual quality labels ('mSQA_min').
...
def compute_specificity_from_cm(cm: np.ndarray) -> np.ndarray:
    ...
    Where:
        TP_k = cm[k, k]
        FP_k = sum over column k except TP_k
        FN_k = sum over row k except TP_k
...
def save_confusion_matrix_artifacts(y_true, y_pred, labels, display_labels, out_prefix: str) -> None:
    ...
    ax.set_title("Step 1 — LogReg (alignment metrics) — OOF Confusion Matrix (%)")
    plt.tight_layout()
...
# class_weight='balanced' compensates for class imbalance by reweighting samples inversely to class frequency.
...
save_confusion_matrix_artifacts(
    ...
    out_prefix="step1_alignment_logres_cv",
)
...
macro_csv = os.path.join(OUT_DIR, "step1_alignment_logres_cv_macro_metrics.csv")
...
per_class_csv = os.path.join(OUT_DIR, "step1_alignment_logres_cv_per_class_metrics.csv")
...
pred_csv = os.path.join(OUT_DIR, "step1_alignment_logres_oof_predictions.csv")
pred_df.to_csv(pred_csv, index=False)

# %% Minimal console summary
print(f"[Step 1] Saved outputs to: {OUT_DIR}")
print(" - Confusion matrix (% PNG):    step1_alignment_logres_cv_confusion_matrix_percent.png")
print(" - CV macro metrics:            step1_alignment_logres_cv_macro_metrics.csv")
print(" - CV per-class metrics:        step1_alignment_logres_cv_per_class_metrics.csv")
print(" - OOF predictions (per-row):   step1_alignment_logres_oof_predictions.csv")
