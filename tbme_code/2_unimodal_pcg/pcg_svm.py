# -*- coding: utf-8 -*-
"""
PCG unimodal SQIs + 3-class RBF SVM (OOF-CV)

Purpose
-------
Train and evaluate a 3-class Support Vector Machine (RBF kernel) classifier using
unimodal PCG SQIs (previously extracted from raw PCG) and manual PCG quality labels
("PCG" column in the manual annotation Excel).
...
# Ignore generic sklearn user warnings.
warnings.filterwarnings("ignore", category=UserWarning)

# %% Global config (edit if your paths differ)

# Extracted PCG SQI CSV from previous step (same location as in LogReg version)
...
OUT_DIR = "exp_step2_pcg_unimodal_allSQI_svm_rbf_3class"
...
def save_confusion_matrix_artifacts(y_true, y_pred, labels, display_labels, out_prefix: str) -> None:
    ...
    Confusion matrix is computed on the provided vectors (here: OOF predictions),
    ...
    ax.set_title("Step 2 — RBF SVM (PCG unimodal SQIs) — OOF Confusion Matrix (%)")
    plt.tight_layout()
...
if "PCG" not in merged.columns:
    raise ValueError("PCG label not found in merged dataframe. Ensure manual Excel contains 'PCG'.")
...
save_confusion_matrix_artifacts(
    ...
    out_prefix="step2_pcg_svm_rbf_cv",
)
...
macro_csv = os.path.join(OUT_DIR, "step2_pcg_svm_rbf_cv_macro_metrics.csv")
...
per_class_csv = os.path.join(OUT_DIR, "step2_pcg_svm_rbf_cv_per_class_metrics.csv")
...
pred_csv = os.path.join(OUT_DIR, "step2_pcg_svm_rbf_oof_predictions.csv")
pred_df.to_csv(pred_csv, index=False)

# %% Minimal console summary
print(f"[Step 2] Saved outputs to: {OUT_DIR}")
print(" - Confusion matrix (% PNG):    step2_pcg_svm_rbf_cv_confusion_matrix_percent.png")
print(" - CV macro metrics:            step2_pcg_svm_rbf_cv_macro_metrics.csv")
print(" - CV per-class metrics:        step2_pcg_svm_rbf_cv_per_class_metrics.csv")
print(" - OOF predictions (per-row):   step2_pcg_svm_rbf_oof_predictions.csv")
