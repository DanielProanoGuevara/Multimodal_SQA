# -*- coding: utf-8 -*-
"""
PCG unimodal SQIs + 3-class Logistic Regression (OOF-CV)

Purpose
-------
Train and evaluate a 3-class Logistic Regression classifier using unimodal PCG SQIs
(previously extracted from raw PCG) and manual PCG quality labels ("PCG" column in the
manual annotation Excel).

The manual 6-point PCG scale is mapped into 3 classes:
    0           -> low_quality
    1, 2        -> uncertain
    3, 4, 5     -> high_quality
...
4) Per-sample OOF predictions saved as CSV with the structure:
   [ID, Auscultation_Point, ...pcg_sqis..., manual_label, predicted_label]

Inputs (relative to this script)
--------------------------------
- AQ_PATH: CSV with extracted PCG SQIs (expects columns 'ID', 'Auscultation_Point', and the SQI columns).
- MQ_PATH: Excel with manual labels (expects columns 'Trial', 'Spot', and 'PCG').
...
# AQ_PATH now points to the extracted PCG SQI CSV from the previous step.
# This script is intended to live at:
#   Multimodal_SQA/tbme_code/2_unimodal_pcg/pcg_logres.py
# so the extracted features are in:
#   Multimodal_SQA/tbme_code/2_unimodal_pcg/exp_step2_pcg_features/step2_pcg_unimodal_features_extracted.csv
...
OUT_DIR = "exp_step2_pcg_unimodal_allSQI_logreg_3class"
...
# SQI feature columns (must match Step 2 extractor output)
PCG_SQI_COLS = [
...
def merge_quality_dataframes(ex1_quality: pd.DataFrame, m_quality: pd.DataFrame) -> pd.DataFrame:
    """
    Merge extracted PCG SQIs (ex1_quality) with manual labels (m_quality).
    ...
    Returns a clean dataframe with:
      ...
      - manual label: PCG (if present)
      - the SQI feature columns listed in PCG_SQI_COLS (if present)
    """
    ...
    # Manual columns to keep (only if they exist)
    manual_keep = ["Trial", "_k_spot"]
    if "PCG" in m.columns:
        manual_keep.append("PCG")
    ...
    # Keep only requested SQI columns that exist in the feature file
    sqi_cols = [c for c in PCG_SQI_COLS if c in merged.columns]

    base_cols = ["ID", "Auscultation_Point"] + (["PCG"] if "PCG" in merged.columns else [])
...
def save_confusion_matrix_artifacts(y_true, y_pred, labels, display_labels, out_prefix: str) -> None:
    ...
    ax.set_title("Step 2 — LogReg (PCG unimodal SQIs) — OOF Confusion Matrix (%)")
    plt.tight_layout()
...
def fold_metrics_from_predictions(y_true, y_pred, y_proba, class_labels) -> dict:
    ...
    Macro metrics
    -------------
    - AUC OvR (multiclass one-vs-rest)
    - Accuracy
...
# %% Load + merge
# Load extracted SQIs (CSV) and manual labels (Excel)
ex1_quality = pd.read_csv(AQ_PATH)
m_quality = pd.read_excel(MQ_PATH)

# Merge into a single table containing manual PCG labels and SQI features
merged = merge_quality_dataframes(ex1_quality, m_quality)

# Manual PCG label is required for supervised training in this step
if "PCG" not in merged.columns:
    raise ValueError(
        "PCG label not found in merged dataframe. Ensure manual Excel contains 'PCG'."
    )

# Select SQI feature columns (strictly the known list; ensures stable model input)
pcg_sqi_cols_present = [c for c in PCG_SQI_COLS if c in merged.columns]
if not pcg_sqi_cols_present:
    raise ValueError(
        "No expected PCG SQI columns found after merge. "
        f"Expected at least one of: {PCG_SQI_COLS}"
    )

# %% Label preprocessing
df = merged.copy()

# Convert manual PCG labels to numeric (invalid entries -> NaN) then drop missing labels
df["PCG"] = pd.to_numeric(df["PCG"], errors="coerce")
df = df.dropna(subset=["PCG"]).copy()

# Map 6-point label into 3-class strings, then drop invalid/unmapped entries
df["manual_label"] = df["PCG"].astype(int).map(RELABEL_MAP)
df = df.dropna(subset=["manual_label"]).copy()

# Remove samples with missing SQIs (must be complete feature vectors)
df = df.dropna(subset=pcg_sqi_cols_present).copy()

# Encode labels into integers using fixed class order
le = LabelEncoder()
le.fit(CLASS_ORDER)
df["y"] = le.transform(df["manual_label"].values)

# Features and targets as numpy arrays for sklearn
X = df[pcg_sqi_cols_present].values
y = df["y"].values.astype(int)

# IDs for per-sample export (keep as strings for stable CSV formatting)
...
# %% Model pipeline
# NOTE: multi_class is intentionally not set to avoid sklearn FutureWarning.
lr_pipe = Pipeline([
...
save_confusion_matrix_artifacts(
    ...
    out_prefix="step2_pcg_logres_cv",
)
...
macro_csv = os.path.join(OUT_DIR, "step2_pcg_logres_cv_macro_metrics.csv")
...
per_class_csv = os.path.join(OUT_DIR, "step2_pcg_logres_cv_per_class_metrics.csv")
per_class_df.to_csv(per_class_csv, index=False)

# %% 4) Per-sample OOF predictions CSV
pred_df = df[["ID", "Auscultation_Point"] + pcg_sqi_cols_present + ["manual_label"]].copy()
pred_df["predicted_label"] = np.array(class_names, dtype=object)[oof_pred]

pred_df = pred_df[["ID", "Auscultation_Point"] + pcg_sqi_cols_present + ["manual_label", "predicted_label"]]
pred_csv = os.path.join(OUT_DIR, "step2_pcg_logres_oof_predictions.csv")
pred_df.to_csv(pred_csv, index=False)

# %% Minimal console summary
print(f"[Step 2] Saved outputs to: {OUT_DIR}")
print(" - Confusion matrix (% PNG):    step2_pcg_logres_cv_confusion_matrix_percent.png")
print(" - CV macro metrics:            step2_pcg_logres_cv_macro_metrics.csv")
print(" - CV per-class metrics:        step2_pcg_logres_cv_per_class_metrics.csv")
print(" - OOF predictions (per-row):   step2_pcg_logres_oof_predictions.csv")
